...
ggplot(states, aes(x = long, y = lat, group = group, fill = le_african)) + geom_polygon(color = "white") + scale_fill_gradient(name = "Years", low = "#ffe8ee", high = "#c81f49", guide = "colorbar", na.value="#eeeeee") + labs(title="Espérance de vie du groupe ethnique Afro-Am???ricain") + coord_map()

# Life expectancy of Caucasian-American
ggplot(states, aes(x = long, y = lat, group = group, fill = le_caucasian)) + geom_polygon(color = "white") + scale_fill_gradient(name = "Years", low = "#ffe8ee", high = "#c81f49", guide = "colorbar", na.value="Gray") + labs(title="Espérance de vie du groupe ethnique Caucasien-Am???ricain") + coord_map()

# Dipslay differencies between life expectancies
ggplot(states, aes(x = long, y = lat, group = group, fill = le_diff)) + geom_polygon(color = "white") + scale_fill_gradient(name = "Years", low = "#ffe8ee", high = "#c81f49", guide = "colorbar", na.value="#eeeeee") + labs(title="Différences dans l'espérance de vie des groupes ethniques \nCaucasien et Afro-Américain par état aux USA") + coord_map()

# Display interactive USA map with contextual message for each state : caucasian-american
map_plot_caucasian = ggplot(states, aes(x = long, y = lat, group = group, fill = le_caucasian)) + geom_polygon(color = "white") + scale_fill_gradient(name = "Years", low = "#ffe8ee", high = "#c81f49", guide = "colorbar", na.value="#eeeeee") + labs(title="Espérance de vie du groupe ethnique Caucasien-Américain") + coord_map()
ggplotly(map_plot_caucasian)

# Display interactive USA map with contextual message for each state : afro-american
map_plot_african = ggplot(states, aes(x = long, y = lat, group = group, fill = le_african)) + geom_polygon(color = "white") + scale_fill_gradient(name = "Years", low = "#ffe8ee", high = "#c81f49", guide = "colorbar", na.value="#eeeeee") + labs(title="Espérance de vie du groupe ethnique Afro-Américain") + coord_map()
