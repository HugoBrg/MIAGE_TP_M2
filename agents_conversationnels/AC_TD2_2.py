...
def actionTraj(emplacement):
    if etat[0] == "A":
        if etat[2] == goal[1]:
            ...
            etat[2] += 1
            print("Objectif non atteint, on nettoie la salle, nous allons en case {0}".format(etat[2]))
            salleA[etat[2]] = 0
...
def setGoal():
    ...
    if salle:
        ...
        goal = ['A',case]
    print("Le but de l'aspirateur est la case {0} dans le salle {1}".format(goal[1],goal[0]))
    return goal
